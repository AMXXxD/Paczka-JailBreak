...
parachute_reset(id)
{
	...
	if (is_user_alive(id))
	  fm_set_user_gravity(id, last_gravity[id])
...
public client_PreThink(id)
{
	...
	if (para_ent[id] > 0 && (flags & FL_ONGROUND))
	{
		...
		{	
			if (fm_get_user_gravity(id) == 0.1) fm_set_user_gravity(id, last_gravity[id])
	...
	if (button & IN_USE)
	{
		...
		{	
			if(para_ent[id] <= 0)
			{
				last_gravity[id] = fm_get_user_gravity(id);
