...
public plugin_init()
{
	...
	register_dictionary("jb_mapchoser.txt");
	sHudObj = CreateHudSyncObj();
	
	static ConfigsDir[64]; 
	get_localinfo("amxx_configsdir", ConfigsDir, 63);
	formatex(MapFile, 63, "%s/jb_mapchoser/mapdatas.dmc", ConfigsDir);
	get_localinfo("amxx_configsdir", ConfigsDir, 63);
	formatex(MapCycle, 31, "mapcycle.txt");
	formatex(Prefix, 31, "!g[PrisonBreak]");
	...
	server_cmd("exec %s/jb_mapchoser/dmc.cfg", ConfigsDir);
	server_exec();
...
public plugin_natives() {
	register_native("jail_startvote", "StartVote", 1);
}
